...
def list_oper():
    ...
    # is 연산자는 두 객체가 메모리에서 같은 객체인지를 비교합니다.
    # == 연산자는 두 리스트의 내용이 같은지를 비교합니다.
    ...
    # append (뒤에 요소 추가) vs extend (뒤에 리스트 연결)
    print(cp)
    cp.append(["Java", True, 3.14159])  # 맨 뒤에 요소 추가
    print(cp)   # list가 아닌 요소 하나로 추가됨
    # [1, 2, 'Python', ['Java', True, 3.14159]]

    cp.extend(["Java", True, 3.14159])  # 맨 뒤에 새 리스트를 확장
    print(cp)
    # [1, 2, 'Python', ['Java', True, 3.14159], 'Java', True, 3.14159]

    cp.insert(2, [1, 2, 3]) # 2번 인덱스에 요소 추가됨
    print(cp)

    # 포함 여부 확인
    print("Python" in cp)

    # cp 내부에서 Python 의 인덱스 확인
...
def list_methods():
    ...
    # 반전 reverse
    print("REVERSED:", list(reversed(lst_copy)))    # reversed : 원본 유지, 데이터 순서 반전 -> 새 리스트 출력
    print("lst_copy:", lst_copy)    # 원본 그대로
    print("REVERSE:", lst_copy.reverse())   # reverse : 내부 데이터를 뒤집는 역할 -> 내부 데이터 변경/None
    print("lst_copy:", lst_copy)    # 데이터 뒤집어짐
