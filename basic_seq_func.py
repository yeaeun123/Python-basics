def using_range():
    """
    range 객체 : 범위 객체,
                순차자료형 (인덱싱,슬라이싱 가능), 변경불가
    - 범위 정보만 가지고 있다가 요청할 때 조건의 계산 결과를 반환
    ...
    # 큰 수 -> 작은 수 : 간격값을 음수로
    seq4 = range(0, -10, -1)    # 0 이하 -10 초과 정수 역순
    ...
    print(seq[0], seq[1], seq[2], seq[3])   # 정방향 인덱스
...
def using_zip():
    """
    zip : 여러 시퀀스 자료를 동시에 루프시켜 튜플로 생성 (Generator)
    """
