def define_tuple():
    ...
    tuple: 리스트와 거의 동일, 변경 불가 자료 (immutable)
        len, 인덱싱, 슬라이싱, 연결, 반복, 포함여부 확인 가능
        내부 요소 변경 불가 !
    """
    tp1 = ()    # 공튜플 (비어있음)
    tp2 = tuple()   # 공튜플 (다른 순차형을 튜플로 변환할때 활용)
    print(tp1, type(tp1))
    print(tp2, type(tp2))

    tp3 = (1,)  # 요소가 1개일 경우 요소 뒤에 , 를 찍어줘야 튜플로 인식!!!
    print(tp3, type(tp3))
...
def tuple_oper():
    ...
    # 포함 여부 : in, not in
    print(5 in tp)  # 5 가 포함?
    print(1 not in tp)  # 1 이 포함 안됨?
...
def packing_unpacking():
    ...
    # a, b, c = tp          # 좌변의 변수 갯수가 적음 -> ValueError
    # a, b, c, d, e = tp    # 좌변의 변수 갯수가 많음 -> ValueError

    # 확장 Unpacking
    a, *b = tp  # 확장 언패킹 기호 : * (list로 묶임)
    print(a, type(a))   # 10 <class 'int'>
    print(b, type(b))   # [20, 30, 'Python'] <class 'list'>

    *a, b = tp
    print(a, type(a))   # [10, 20, 30] <class 'list'>
